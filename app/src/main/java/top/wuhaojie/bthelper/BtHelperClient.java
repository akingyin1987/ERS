...
//    private OutputStream mAcceptOutputStream;
    ...
    public void searchDevices(OnSearchDeviceListener listener) {
      ...
      Set<BluetoothDevice> bluetoothDeviceSet =  mBluetoothAdapter.getBondedDevices();
        System.out.println("bonde=="+bluetoothDeviceSet.size());
        if(bluetoothDeviceSet.size()>0){
           mBondedList.addAll(bluetoothDeviceSet);
        }
    ...
    private   void    checkBlueTooth(Context  context){
        ...
        if(null == bluetoothAdapter){
            Toast.makeText(context,"本地蓝牙不可用",Toast.LENGTH_SHORT).show();
            return;
        }
    }
